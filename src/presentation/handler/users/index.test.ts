...
describe('Users', () => {
  ...
  beforeEach(async () => {
    ...
    let res = await request(app).post('/auth/login').send({ email: adminUser.email, password: userPassword });
    adminUserToken = res.body.data;
    res = await request(app).post('/auth/login').send({ email: standardUser.email, password: userPassword });
  ...
  describe('GET /auth/users', () => {
    ...
    it('should report error of unauthorized user', async () => {
      const res = await request(app).get('/users').set('Authorization', standardUserToken);
  ...
  describe('GET /auth/users//:id([0-9]+)', () => {
    it('should get user', async () => {
      const user = await userRepository.findOne({ email: adminUser.email });
      const res = await request(app).get(`/users/${user.id}`).set('Authorization', adminUserToken);
      expect(res.status).to.equal(200);
